...
const [min, max] = range.split('to').map(str => Number(str.trim()));
...
return selectedFilters.propertyTypes.includes(property.propertyType);
};
...
const filteredProperties = useMemo(() => {
  return properties
    .filter(p => matchesType(p) && matchesPrice(p) && matchesSearch(p) && matchesHeroDestination(p))
    .sort(sortProperties);
}, [properties, selectedFilters, selectedSort, searchQuery, heroDestination]);


return ( <div className='bg-gradient-to-r from-[#fffbee] to-white py-16 pt-28'> <div className='max-padd-container flex flex-col sm:flex-row gap-8 mb-16'>
{/* Left side Filters */} <div className='bg-secondary/10 ring-1 ring-slate-900/5 p-4 sm:min-w-60 sm:h-[600px] rounded-xl'>
{/* Sort by price */} <div className='py-3 mt-4'> <h5 className='h5 mb-3'>Sort By</h5>
<select
value={selectedSort}
onChange={(e) => setSelectedSort(e.target.value)}
className='bg-secondary/10 border border-slate-900/10 outline-none text-gray-50 medium-14 h-8 w-full rounded px-2'
>
{sortOptions.map((sort, index) => ( <option key={index} value={sort}>{sort}</option>
))} </select> </div>
        ...
        {propertyTypes.map(type => (
          <label key={type} className='flex gap-2 medium-14'>
            <input
              ...
              onChange={(e) => handleFilterChange(e.target.checked, type, 'propertyTypes')}
            />
            {type}
          </label>
        ))}
