...
{ headers: { Authorization: `Bearer ${await getToken()}` } }
  ...
  if (data.success) {
    ...
    getProperties(); // Refresh list after toggle
  } else {
    toast.error(data.message);
  }
} catch (error) {
...
return (
   <div className="md:px-8 py-6 xl:py-8 m-1 sm:m-3 h-[97vh] overflow-y-scroll lg:w-11/12 bg-white shadow rounded-xl"> <div> <div className="flex justify-between flex-wrap gap-2
        sm:grid-cols-[2fr_2fr_1fr_1fr] lg:grid-cols-[0.5fr_2fr_2fr_1fr_1fr]
        px-6 py-3 bg-secondary border-b-1 border-slate-900/15 rounded-t-xl"> <h5 className="h4 hidden lg:block">Index</h5> <h5 className="h4">Name</h5> <h5 className="h4">Address</h5> <h5 className="h4">Price</h5> <h5 className="h4">Available</h5> </div>
